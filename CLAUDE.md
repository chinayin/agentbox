...
- 构建定义只有 `docker-bake.hcl` 一份，`make image`、`ci.yml`、`release.yml` 都调它。要加 target、平台或 build arg 就改它，不要在 Makefile 或 workflow 里另写一条 `docker build`。调 bake 必须带 `-f docker-bake.hcl`，代理变量必须带 `BUILD_` 前缀，两条都由 `test.sh` 的 `workflow invariants` 组看着。
