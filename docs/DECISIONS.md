...
| 2026-09-21 | examples | 原 aliyun 样例（单云、AK 走 env 变量、无 profile）删除，换成 `examples/multicloud`：从真实实例脱敏，凭据全部是 `home/` 里的 profile 文件、`accounts.yaml` 名册、四个 subagent 与工作区提示词。env 里放云 AK 的写法与「每条命令必须带 `--profile`」冲突，不再示范 |
| 2026-09-22 | GHCR 与 git tag | 只保留 0.7.1：删除 GHCR `agentbox` 包 0.3.0 到 0.7.0 共 34 个版本（含每个 tag 下的 per-arch 与 attestation 子版本）、`agentbox-buildcache` 5 个无 tag 的旧 blob，远端与本地 tag `v0.3.0` 到 `v0.7.0`。用 `gh api -X DELETE /user/packages/container/<pkg>/versions/<id>`，按 id 逐个点名；没有 GitHub Release 需要处理 |
| 2026-09-22 | GitHub push protection | `AKID` 加 32 个 `x` 的占位符被当成腾讯云 SecretId 拒推（GH013），扫描只看形状。样例里的占位符必须短于真实格式，`scripts/test.sh` 盯着 AKID/AKIA/LTAI 三种形状 |
