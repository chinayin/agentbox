...
| 9 | `new-instance` 与 `import-instance` 改产出 `home/` | 2026-09-21 文件型凭据的标准改为实例目录 `home/` → `/agent/home`（`docs/TOOLS.md` §1，litellm-gateway 已按此上线）。两个技能仍产出旧式单文件挂载：`scaffold.sh --mount kubeconfig` 写 `/agent/kubeconfig:ro` 加 `KUBECONFIG`，`import-instance` 把源主机的 `~/.ssh`、`~/.kube` 拆成 `ssh_key`、`kubeconfig-*` | `scaffold.sh` 去掉 `--mount`，模板 compose 带 `./home:/agent/home:ro`；`import-instance` 把源主机家目录里的凭据原样落到 `home/`；`test.sh` 对应断言改写 |
