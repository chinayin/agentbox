...
**什么时候不该用 subagent。** subagent 的工具调用与推理只留在它自己的上下文里，主 bot 只拿到最后一段报告，所以 cc-connect 无论怎么回显，飞书里都看不到 subagent 跑了什么。它换来的只有两样：跨账号、跨云的并行，以及把巡检刷出的大段 CLI 输出挡在主会话之外。单账号单任务用它是纯亏：多一次模型往返，过程还被藏起来。主 bot 的路由规则因此是：

- 单家云、单个账号的查询，主 bot 自己带 `--profile` 跑，命令与输出留在主会话里，人在飞书里看得见。
- 只有要对多个账号或多家云扇出，或者预期输出会撑爆主会话时，才派 subagent，每个 subagent 一个 profile。
- 不同话题里同时问不同的云不需要 subagent：`thread_isolation = true` 已经让每个话题各起一个会话，并行是 cc-connect 层的事。共享的是家目录，profile 文件只读没有冲突，CLI 写回家目录的 STS 缓存是否会互相踩，见 [CLOUD_ACCOUNTS §7](CLOUD_ACCOUNTS.md) 的未验证项。
