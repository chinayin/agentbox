...
1. **Copy the template into the deploy repo.** `examples/<name>/` is a template, not a running
   instance: copy it into the private deploy repository at `hosts/<host>/instances/<name>/` (see
   `.claude/skills/deploy/references/deploy-repo.md`), rename `env.example` to `env` there, and
   `chmod 600` it. Fill in the real values in the deploy repo, not in this one.
...
7. **pi only.** `agent.type = "pi"` needs the `-pi` image (the snippet already uses
