...
| probe egress shows `000` or long `time_total` for one upstream | That upstream is blocked or throttled from the host | Build waits or fails on that tool only; try later or use `make image BUILD_ARGS=--build-arg HTTPS_PROXY=...` on the host (`docs/CN_MIRRORS.md` §1) |
