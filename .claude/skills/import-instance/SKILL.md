...
`import` 的 stdout 只有规划表；stderr 最后一行给出下一步命令。实例的 `docker-compose.yaml` 直接写进目标目录（从 `examples/demo/docker-compose.yaml` 改名而来，文件型凭据全部走模板自带的 `./home:/agent/home:ro`，技能清单另一条挂载），不再有需要手贴的片段。真值只在 `instances/<name>/` 下的文件里，规划表与 compose 文件里没有任何值。
...
3. `home/.kube/` 与 `home/.ssh/` 里只保留目标主机该有的文件；删掉 kubeconfig 时同步改 `config.toml` 的 `KUBECONFIG`（只剩 `config` 一份就整行删掉）。
